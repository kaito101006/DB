...
const  Subject2 =()=> {
  ...
  const current_getData = gData.filter((getData)=> {
    if(page === 1) {
    return getData.ID ==="1";
    }else if(page === 2) {
      return getData.ID ==="2";
    }else{
      return getData.ID === "999";
    }
  });
...
<div ClassName="get1">
  {current_getData.map((getData)=>(
       
    <li  id="json" key={getData.ID}style={{listStyle:'none'}}><output id="op1">コード：{getData.Code}　パスワード：{getData.password}</output></li>
    ))}
    <button className="bu1"  onClick = {prev}>＜</button>
    <button className="bu2"  onClick= {next}>＞</button>
...
<aside>
 ...
 <li className="li5"><a href="#nz6">DB</a></li>
 </ul>
